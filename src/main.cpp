...
float g_CamDistanceX = 0.50f;
float g_CamDistanceY = 0.5f;
float g_CamDistanceZ = 2.5f;

// flags para teclas walkaround
int W_presionado =0, S_pressionado=0, A_pressionado=0, D_pressionado=0;
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 Trab final FCG - Gabriel e José", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // salvando os valores do seno e cosseno para economizar
        ...
        if (W_presionado)
        {
            g_CamDistanceY -= (velocidade*sin_g_CameraPhi);
            g_CamDistanceZ -= (velocidade*cos_g_CameraPhi * cos_g_CameraTheta);
            g_CamDistanceX -= (velocidade*cos_g_CameraPhi * sin_g_CameraTheta);
        }
        ...
        if (A_pressionado)
        {
            g_CamDistanceX -= (velocidade*cos_g_CameraPhi * cos_g_CameraTheta);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if (key == GLFW_KEY_W)
        W_presionado = (action == GLFW_RELEASE) ? 0 : 1;

    if (key == GLFW_KEY_S)
        S_pressionado = (action == GLFW_RELEASE) ? 0 : 1;

    if (key == GLFW_KEY_A)
        A_pressionado = (action == GLFW_RELEASE) ? 0 : 1;

    if (key == GLFW_KEY_D)
        D_pressionado = (action == GLFW_RELEASE) ? 0 : 1;
